...
wheel :: [Colour Double] -> Diagram B V2 Double
...
solar :: Angle Double -> Diagram B V2 Double
...
loop context n = do
  send context $ renderDia Canvas (CanvasOptions (Width 500)) 
                                  (solar (n/288 @@ turn))
  threadDelay 20000
  loop context (n + 1)
